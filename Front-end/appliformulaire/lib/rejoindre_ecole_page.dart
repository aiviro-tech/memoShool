        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              if (_etape == 3)
                Expanded(
                  child: _EtapeInfosComplementaires(
                    role: _roleChoisi,
                    code: _codeVerifie,
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        ...
                        color: estActif || estPasse
                            ...
                            : Colors.grey,
                        fontWeight:
                            estActif ? FontWeight.bold : FontWeight.normal,
                      ),
                    ),
                  ],
                ),
...
class _EtapeSaisieCodeState extends State<_EtapeSaisieCode> {
  ...
  String get _prefixeAttendu {
    switch (widget.role) {
      case 'etudiant':  return 'ETU-';
      case 'enseignant': return 'ENS-';
  ...
  Color get _couleurRole {
    switch (widget.role) {
      case 'etudiant':  return Colors.blue;
      case 'enseignant': return Colors.green;
      case 'admin':     return Colors.orange;
      default:          return app.AppColors.primary;
    }
  }
  ...
  void _verifierCode() async {
    ...
    setState(() { _isLoading = true; _erreur = ''; });
    try {
      ...
      if (!mounted) return;
      setState(() { _isLoading = false; _erreur = message; });
    } catch (_) {
      if (!mounted) return;
      setState(() {
        _isLoading = false;
      ...
      children: [
          ...
          Row(
            children: [
              const Icon(Icons.error_outline,
                  color: app.AppColors.red, size: 16),
              const SizedBox(width: 6),
              Expanded(
                child: Text(
                  _erreur,
                  style:
                      const TextStyle(color: app.AppColors.red, fontSize: 12),
                ),
              ),
            ],
          ),
          ...
          children: [
            Expanded(
              child: OutlinedButton(
                ...
                child: const Text(
                  "Retour",
                  style: TextStyle(color: app.AppColors.primary),
                ),
              ),
            ),
        ...
        Container(
          ...
          child: const Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Icon(Icons.info_outline, color: app.AppColors.primary, size: 18),
              SizedBox(width: 10),
              Expanded(
                child: Text(
                  ...
                  style:
                      TextStyle(fontSize: 12, color: app.AppColors.primary),
                ),
              ),
            ],
          ),
...
class _EtapeInfosComplementairesState
  ...
  void _soumettreDemande() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(message), backgroundColor: app.AppColors.red),
      );
    } catch (_) {
      if (!mounted) return;
      setState(() => _isLoading = false);
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            style:
                const TextStyle(color: app.AppColors.textSub, fontSize: 13),
          ),
          const SizedBox(height: 24),
              ...
              Expanded(
                child: OutlinedButton(
                  ...
                  style: OutlinedButton.styleFrom(
                    padding: const EdgeInsets.symmetric(vertical: 14),
                    side:
                        const BorderSide(color: app.AppColors.primary),
                    shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(10),
                    ),
                  ),
  ...
  Widget _champsEtudiant() {
    return Column(
      children: [
        _dropdown("Niveau / Classe *", _niveaux, _niveauSelectionne,
            (v) => setState(() => _niveauSelectionne = v),
            obligatoire: true),
        const SizedBox(height: 14),
        _champ("Numéro matricule", _matriculeController),
        const SizedBox(height: 14),
        _dropdown("Année d'entrée *", _annees, _anneeEntreeSelectionnee,
            (v) => setState(() => _anneeEntreeSelectionnee = v),
            obligatoire: true),
        const SizedBox(height: 14),
        _dropdown("Statut *", _statuts, _statutSelectionne,
            (v) => setState(() => _statutSelectionne = v),
            obligatoire: true),
        const SizedBox(height: 14),
        _champ("Nom du tuteur", _tuteurNomController),
        const SizedBox(height: 14),
        _champ("Téléphone du tuteur", _tuteurTelController,
  ...
  Widget _champsEnseignant() {
    return Column(
      children: [
        ...
        const SizedBox(height: 14),
        _champ("Diplôme le plus élevé *", _diplomeController,
            obligatoire: true),
        const SizedBox(height: 14),
        _champ("Années d'expérience *", _experienceController,
            obligatoire: true, clavier: TextInputType.number),
        const SizedBox(height: 14),
        _dropdown("Type de contrat *", _contrats, _contratSelectionne,
  ...
  Widget _champsAdmin() {
    return Column(
      children: [
        _champ("Poste / Fonction *", _posteController,
            obligatoire: true, hint: "ex: Comptable, Secrétaire"),
        const SizedBox(height: 14),
        _dropdown("Service d'affectation *", _services, _serviceSelectionne,
            (v) => setState(() => _serviceSelectionne = v),
            obligatoire: true),
        const SizedBox(height: 14),
        _dropdown("Niveau d'accès *", _niveauxAcces, _niveauAccesSelectionne,
...
class _AttentePage extends StatelessWidget {
  ...
  //  CORRECTION : vider la session + appel logout API + redirection
  //    vers la VRAIE page Connexion de main.dart
